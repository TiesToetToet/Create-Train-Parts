...
//                scene.world().modifyKineticSpeed(util.select().fromTo(25, 0, 4, 26, 1, 4), f -> -f);
    ...
    public static void modes(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.world().showIndependentSection(util.select().fromTo(5, 0, 3, 5, 1, 3), Direction.DOWN);
        scene.world().showIndependentSection(util.select().fromTo(4, 1, 3, 2, 1, 3), Direction.DOWN);
        ...
        scene.overlay().showText(50)
                ...
                .placeNearTarget()
                .attachKeyFrame()
                .text("In Normal mode, players can jump over the gate");
        ...
        scene.overlay().showText(50)
                ...
                .placeNearTarget()
                .attachKeyFrame()
                .text("But this can be configured with a wrench");

        scene.overlay().showControls(util.vector().centerOf(5, 3, 3), Pointing.DOWN, 75)
                .rightClick()
                .withItem(AllItems.WRENCH.asStack());
        scene.overlay().showControls(util.vector().centerOf(6, 2, 3), Pointing.RIGHT, 30).showing(AllIcons.I_CROSSING_NO_BARRIER);

        scene.idle(35);

        scene.overlay().showControls(util.vector().centerOf(6, 2, 3), Pointing.RIGHT, 40).showing(AllIcons.I_CROSSING_BARRIER);
//        scene.world().showIndependentSection(util.select().fromTo(2, 2, 2, 4, 2, 2), Direction.DOWN);

        scene.idle(60);
        scene.overlay().showOutlineWithText(util.select().fromTo(2, 2, 3, 4, 2, 3), 50)
                .placeNearTarget()
                .colored(PonderPalette.RED)
                .pointAt(util.vector().of(3, 4, 2))
                .text("The barrier is made of invisible collision blocks");
//        scene.
